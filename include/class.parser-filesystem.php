<?
    class Parser {
        ...
        function read_loglines($start_line='', $end_line='') {
            $this->logfile_handle = fopen($this->logfile, "r");
            if(!$this->logfile_handle) {
                die("couldn't open logfile ".$this-logfile);
            }
            
            $current_fight_start_line = -1;
            $line_no = -1;
            
            while(($line = fgets($logfile_handle)) !== false) {
                $line_no++;
                $skip=0;
                if($start_line!='' && $line_no<$start_line) {
                    $skip=1;
                }
                if($end_line!='' && $line_no>$end_line) {
                    return;
                }
                        ...
                        if(preg_match('/([^\{]*)\s*\{(.*?)\}\s*:\s*([^\{]*)\s*\{(.*?)\}/', $effect_raw, $matches_effect)) {
                            ...
                            if($effect_id) {                           
                                if($effect_id == FIGHT_START) {
                                    $this->infight = 1;
                                    $this->fight_count++;
                                    $current_fight_start_line = $line_no;
